...
import { useForm16List, useUploadForm16, useToggleForm16Status, useDeleteForm16, useBulkUploadForm16 } from "./hooks/useForm16";
...
export default function Form16Page() {
  ...
  const { data: form16List = [], isLoading } = useForm16List(isAccountant);
  const toggleMutation = useToggleForm16Status();
  const deleteMutation = useDeleteForm16();
  const uploadMutation = useUploadForm16(() => {
  ...
  const bulkUploadMutation = useBulkUploadForm16(
    (data) => {
      setBulkResult(data);
      setBulkFile(null);
  ...
  const handleBulkSubmit = (e) => {
    e.preventDefault();
    if (!bulkYear) return toast.error("Please select an academic year.");
    if (!bulkFile) return toast.error("Please select a ZIP file.");

    const formData = new FormData();
    formData.append("academic_year", bulkYear);
  ...
  const handleDownload = async (id, empCode, academicYear) => {
    try {
      toast.loading("Downloading PDF...", { id: "pdf-dl" });
      const blob = await form16Service.downloadForm16Admin(id);
      const blobUrl = URL.createObjectURL(blob);
      
      const link = document.createElement("a");
      link.href = blobUrl;
      link.download = `Form16_${empCode}_${academicYear.replace('/', '-')}.pdf`;
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this Form 16 record permanently?")) {
      deleteMutation.mutate(id);
    }
  };

  // Search filter
  const filteredForm16List = useMemo(() => {
    if (!q) return form16List;
    const query = q.toLowerCase();
    return form16List.filter(item => {
      const emp = item.employee_id || {};
      return (
        (emp.full_name || "").toLowerCase().includes(query) ||
        (emp.employee_code || "").toLowerCase().includes(query) ||
        (item.academic_year || "").toLowerCase().includes(query)
      );
    });
  }, [form16List, q]);
    ...
    <TooltipProvider>
      <div className="space-y-6">
        <div className="flex items-end justify-between flex-wrap gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.18em] text-accent">Tax Documents</p>
            <h1 className="font-display text-4xl text-primary mt-1">Form 16</h1>
            <p className="text-sm text-muted-foreground mt-2">Upload and manage annual Form 16 (TDS Certificate) PDFs for university employees.</p>
          </div>
          <div className="flex gap-2">
            <Dialog open={isBulkOpen} onOpenChange={(open) => {
              ...
              <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
                <DialogHeader>
                  <DialogTitle>Bulk Upload Form 16 (ZIP)</DialogTitle>
                  <CardDescription>
                ...
                {!bulkResult ? (
                  <form onSubmit={handleBulkSubmit} className="space-y-4 py-2">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="border border-dashed rounded-lg p-6 flex flex-col items-center justify-center gap-2 hover:bg-muted/30 transition-colors cursor-pointer relative">
                        <input
                          type="file"
                          accept=".zip"
                          required
            ...
            <Dialog open={isUploadOpen} onOpenChange={setIsUploadOpen}>
              <DialogTrigger asChild>
                <Button size="sm">
                  <Plus className="h-4 w-4 mr-2" /> Upload Form 16
                </Button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  <DialogTitle>Upload Form 16 PDF</DialogTitle>
                  <CardDescription>
                    Upload an official TDS certificate PDF file for an employee.
                  </CardDescription>
                </DialogHeader>
                <form onSubmit={handleUploadSubmit} className="space-y-4 py-2">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="border border-dashed rounded-lg p-4 flex flex-col items-center justify-center gap-2 hover:bg-muted/30 transition-colors cursor-pointer relative">
                      ...
                      <span className="text-sm font-medium text-muted-foreground">
                        {file ? file.name : "Select signed Form 16 PDF"}
                      </span>
                      <span className="text-xs text-muted-foreground/60">
                        Only PDF files allowed (Max 100KB)
                      </span>
                    </div>
                  </div>
        ...
        <div className="border rounded-lg bg-card overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {isLoading && (
                <TableRow>
                  <TableCell colSpan={7} className="text-center text-muted-foreground py-8">
                    Loading Form 16 database records...
                  </TableCell>
                </TableRow>
              )}
              {!isLoading && filteredForm16List.length === 0 && (
                <TableRow>
                  <TableCell colSpan={7} className="text-center text-muted-foreground py-8">
              ...
              {filteredForm16List.map((item, idx) => {
                const emp = item.employee_id || {};
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => handleDownload(item._id || item.id, emp.employee_code, item.academic_year)}
                            >
